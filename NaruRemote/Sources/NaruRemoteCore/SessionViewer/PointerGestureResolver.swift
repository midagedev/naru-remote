...
public struct PointerGestureResolver: Sendable {
    ...
    /// instead of waiting for a reveal edge. The cursor sensitivity
    /// compensates this coupling so visible cursor travel remains
    /// finger-paced while the viewport follows more like a native
    /// photo/map surface.
    ...
    private func smoothedAutoPanTransform(
        ...
        // samples. If the reveal step consumes too much of the sample, the
        // visible cursor looks delayed even though the viewport is moving.
        // Cap reveal below half the touch delta so the cursor still visibly
        // follows the finger while the viewport catches up.
