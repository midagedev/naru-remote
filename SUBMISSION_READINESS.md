...
10. ✅ **스토어 스크린샷 재촬영 완료(2026-08-18)**: 폐기된 Direct 모드 컷을
   ...
   `docs/store-screenshots.md`. **업로드는 다크 세트로 한다** — 라이트 모드는
   라이브 세션 위 독의 대비 결함(아래) 때문에 비교용으로만 남긴다.
...
11. ⚠️ **라이트 모드 독 대비 결함(스토어 촬영 중 발견)**: 라이브 세션 위
   독이 `.ultraThinMaterial`이라 어두운 원격 화면 위에서 중간 회색으로
   해석되는데 상태 문구는 `.secondary`(라이트에서 진회색)라 대비가 약 2:1까지
   내려간다(WCAG AA 4.5:1). 다크 모드는 영향 없다. 구조적 수정 방향과 재발
   방지 게이트는 `NEXT_STEPS.md` P0 #6에 기록했다. 심사 반려 사유는 아니지만
   라이트 모드 사용자의 1차 입력면 가독성 문제다.
