...
Current local result after the action-order correction:
...
- VNC 10fps gate:
  ...
  - content FPS: `1.98`
  - average update: `505` ms
  - p95 update: `628` ms
  - first-byte wait p95: `619` ms
  - payload-read p95: `0` ms
  - client-processing p95: `8` ms
...
architecture changes. Screen Recording now precedes physical iPhone setup in
the recommended action order because the true helper-video live gate cannot run
until the helper app is allowed to capture the screen:

1. Grant Screen Recording permission to the stable helper app bundle.
2. Rerun the helper ScreenCaptureKit probe.
3. Run the true helper-video live capture benchmark.
4. Make the physical iPhone available to Xcode and then run the physical
