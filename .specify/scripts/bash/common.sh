...
get_current_branch() {
    ...
    if has_git; then
        ...
        if symbolic_branch=$(git -C "$repo_root" symbolic-ref --quiet --short HEAD 2>/dev/null); then
            echo "$symbolic_branch"
            return
        fi

        local parsed_branch
        if parsed_branch=$(git -C "$repo_root" rev-parse --abbrev-ref HEAD 2>/dev/null); then
            echo "$parsed_branch"
            return
        fi
    fi
