...
- **IN-002 Remote injection behavior**: pointer taps/clicks/right-clicks/drag/scroll emit RFB `PointerEvent` (RFC 6143 §7.5.5) via the existing `RFBPointerEventClient`; quick keys emit `KeyEvent` (§7.5.4) via the existing `KeystrokeEmitter`. Zoom/pan emit nothing.
