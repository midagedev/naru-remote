...
- **FR-016**: Benchmark promotion for poor-network usability MUST treat traffic pressure as a first-class target. Request-region candidates MUST report redacted framebuffer-relative `requestRegionAreaPermille` and, when the first non-incremental request is not the production full-frame default, `firstFrameRequestAreaPermille` traffic proxies alongside hit-rate, latency, dirty-area, first-frame startup latency, first-frame receive timing, and renderer-upload aggregates; reports MUST NOT emit raw byte counts, framebuffer dimensions, coordinates, pixels, or payloads. A benchmark MAY request the first non-incremental frame for a fixed visible-region label, but the production default MUST remain full-frame until the same candidate preserves startup survival, full-frame fallback/heartbeat behavior, usable-run count, and tail latency. A production request-region default must not be promoted solely on FPS if it increases unanswered requests, timeout/failure labels, startup delay, first-frame request area, sustained request area, first-frame payload pressure, or tail latency.
- **FR-017**: An opt-in app stream profile MAY apply viewport-aware regions to incremental framebuffer requests after the first full-frame request. The profile MUST use the existing safe `ViewportTransform` mapping, retain a periodic full-frame heartbeat/fallback path, disable the behavior when power saver overrides the stream profile, and surface only the fixed stream-profile label in settings/diagnostics; it MUST NOT log, export, or persist framebuffer dimensions, coordinates, byte counts, pixels, or per-sample timings.
...
- **ViewportRequestRegionPolicy** — pure, in-memory mapping from `ViewportTransform` to an optional `RFBFramebufferUpdateRegion` for opt-in app profiles and benchmark candidates. It expands the visible region by a safety margin, rejects low-savings near-full regions, and can return full requests for heartbeat/fallback without logging raw coordinates.
...
- **SC-007**: Viewport-aware request-region candidates are considered promotion-ready for poor network conditions only when the benchmark shows a meaningful requested-area reduction versus `full` for both sustained requests (`requestRegionAreaPermille`) and any benchmark-only first-frame visible startup (`firstFrameRequestAreaPermille`). First-frame experiments record only the fixed `streamShapeFirstFrameRequestMode` label plus redacted permille ratios. The candidate must match or improve the incumbent's first-frame startup band, usable-run count, content FPS band, hit-rate band, p95 update tail, and failure-label profile. A candidate that saves area but destabilizes startup or the sustained stream remains a research result, not a production default.
- **SC-008**: Sustained remote-desktop smoothness is not product-ready until a physical-iPhone or live benchmark gate reaches at least `10` content FPS under the relevant target. `iphone-remote-desktop-10fps-v1` treats content FPS below `10` as failure and should be used before claiming Chrome-Remote-like hand feel; lower-FPS VNC results remain diagnostic evidence, not usability success.
