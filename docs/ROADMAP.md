...
- Core viewer (`specs/001`): profiles + Keychain `credentialRef`, staged
  ...
  PiP Watch, fake RFB server harness.
- Session experience (`specs/003`), RFB encodings (`specs/004` —
  CopyRect/Hextile/ZRLE + pseudo-encodings, live-verified against macOS
  Screen Sharing), connection grid diagnostics (`specs/005`).
- Direct Keystroke Streaming Mode (`specs/002`, PRs #28–#35).
- Naru Helper host bridge: text `nativeInsert` (`specs/006`, live-verified
...
- File drop — deferred
- ✅ Naru Helper native insertion — implemented via `specs/006` (helper text
  bridge) and `specs/009` (Live type-through delivery-ladder tier 1); see
  Phase 11
...
## Phase 11 - Host Helper And CRD-Parity Input/Video (2026-07)
...
- `specs/006` helper text bridge: HMAC-paired localhost bridge (`--listen`,
  ...
  with Korean against a real Mac. Empirical hard rule: Unicode X11 keysyms
  are measured `no-input` on macOS Screen Sharing — multilingual text can
  only cross via helper nativeInsert or clipboard-paste, never key events
- `specs/007` helper video stream: `--video-listen` (port 5975, macOS Screen
  Recording permission) implemented; physical iPhone gate passed with a
  synthetic source. Residual: the real-screen gate needs a one-time Screen
  Recording approval on the Mac (`scripts/run-naru-live-benchmark.sh
  helper-dev-app-setup`), then a `physical-iphone-helper-video-gate` re-run
- `specs/009` Live type-through: end-anchored grapheme-cluster diff window,
...
- `specs/010` helper onboarding: 5-step in-app pairing sheet, CSPRNG secret
  ...
  environment indirection (secrets never on argv)
- NaruHelper hardening: `--token` rejected on listeners; `--listen`
  silent-death bug fixed (async park instead of `RunLoop.main.run()`)
- Remaining work is tracked in `NEXT_STEPS.md`: physical gates, real-screen
  video gate, D3 promotion, helper production packaging (menu-bar app,
  notarization, launchd)
